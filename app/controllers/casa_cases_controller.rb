class CasaCasesController < ApplicationController
  ...
  def show
    ...
    respond_to do |format|
      ...
      format.xlsx do
        response.headers["Content-Disposition"] = "attachment; filename=#{casa_case_filename(@casa_case)}"
      end
    end
  end
  ...
  def update
    ...
    if @casa_case.update_cleaning_contact_types(casa_case_update_params)
      ...
      redirect_to edit_casa_org_casa_case_path(current_organization, @casa_case), notice: "CASA case was successfully updated.#{changed_attributes_list}"
    else
      render :edit
    end
  end
