      ...
      describe "invalid request" do
        ...
        context "with case_court_orders_attributes being passed as a parameter" do
          ...
          it "Creates a new CasaCase, but no CaseCourtOrder" do
            expect { post casa_org_casa_cases_url, params: invalid_params }.to change(
              CasaCase,
              :count
            ).by(1)

            expect { post casa_org_casa_cases_url, params: invalid_params }.not_to change(
              CaseCourtOrder,
              :count
            )
          end
      ...
      describe "court orders" do
        context "when the user tries to make an existing order empty" do
          ...
          it "does not update the first court order" do
            expect { patch casa_org_casa_case_url(casa_case), params: {casa_case: orders_updated} }.not_to(
              change { casa_case.reload.casa_org_casa_case_url[0].text }
            )
          end

          it "does not update the second court order" do
            expect { patch casa_org_casa_case_url(casa_case), params: {casa_case: orders_updated} }.not_to(
              change { casa_case.reload.casa_org_casa_case_url[1].text }
            )
          end
        end
      end
